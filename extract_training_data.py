...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack) < 3:
            input = state.stack[::-1] + [None] * (3-len(state.stack))
        else:
            input = state.stack[:-4:-1]
        
        if len(state.buffer) < 3:
            input += state.buffer[::-1] + [None] * (3-len(state.buffer))
        else:
            input += state.buffer[:-4:-1]
        ...
        for i in input:
            if i is None:
                ...
                encodings.append(3)
            elif words[i].lower() not in self.word_vocab:
                encodings.append(2)
            elif pos[i] == 'NNP':
                encodings.append(1)
            elif pos[i] == 'CD':
                ...
                encodings.append(self.word_vocab[words[i].lower()])
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        output = np.zeros(91)
        output[self.output_labels[output_pair]] = 1
        return output
