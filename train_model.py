...
class DependencyModel(Module): 
  ...
  def __init__(self, word_types, outputs):
    ...
    self.hidden = Linear(768, 128)
    self.output = Linear(128, 91)

  def forward(self, inputs):
    # TODO: complete for part 3
    embedded = self.embedding(inputs)
    flat_embedded = embedded.view(embedded.size(0), embedded.size(1) * embedded.size(2))
    hidden_output = self.hidden(flat_embedded)
    activated_hidden = relu(hidden_output)
